...
def test_left_right_advection_symmetry(scheme, dim):
    left_vel = dict(vx=0, vy=0, vz=0) | {f"v{dim}": 1}
    right_vel = dict(vx=0, vy=0, vz=0) | {f"v{dim}": -1}
    mesh = dict(nx=1, ny=1, nz=1) | {f"n{dim}": 64}

    sim_left = HydroSolver(
        ic=partial(ic.square, rhomin=1, rhomax=2, **left_vel),
        **mesh,
        cupy=CUPY_AVAILABLE,
        **scheme,
    )
    sim_right = HydroSolver(
        ic=partial(ic.square, rhomin=1, rhomax=2, **right_vel),
        **mesh,
        cupy=CUPY_AVAILABLE,
        **scheme,
    )

    sim_left.take_n_steps(
        10,
        time_integrator=(
            TimeIntegrator.MUSCL_HANCOCK
            if scheme.get("use_MUSCL", False)
    ...
    idx = sim_left.params.variable_index_map
    assert (
        linf_norm(
            sim_left.snapshot_history[-1].u[idx("rho", keepdims=True)]
            - np.flip(
                sim_right.snapshot_history[-1].u[idx("rho", keepdims=True)], axis=DIM_TO_AXIS[dim]
            )
        )
        < 1e-14
    )
