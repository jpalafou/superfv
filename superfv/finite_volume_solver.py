...
from typing import Callable, Dict, Literal, Optional, Set, Tuple, Union, cast
...
class FiniteVolumeSolver(ExplicitODESolver, ABC):
    ...
    def _init_ic(
        ...
        if ic_passives is not None:
            ...
            self.array_slicer.create_var_group(
                "user_defined_passives", self.user_defined_passive_vars
            )
        ...
        selected_actives = set(test_arr[self.array_slicer("actives")].tolist())
        selected_passives = (
            set(test_arr[self.array_slicer("passives")].tolist())
            if "passives" in self.array_slicer.group_names
            else None
        )
        ...
        if (
            ...
            and not selected_actives | selected_passives == all_selected_vars
        ):
            raise ValueError(
