...
def compute_theta(
    ...
    Args:
        ...
        out: Array to which theta is assigned. Has shape (nvars, nx, ny, nz, 1).
    ...
    # allocate arrays
    dmp = xp.empty(u.shape + (2,))
    node_mp = xp.empty(u.shape + (2,))
    ...
    modified = compute_dmp(xp, u, active_dims, include_corners, dmp)
    ...
    # assign theta
    out[modified[:-1] + (0,)] = theta[modified[:-1]]

    return modified[:-1] + (slice(None, 1),)
